...
// Compact chevron button shown next to the greeting. Tap to switch between children.
...
export default function InlineChildSwitcher() {
  ...
  if (!activeChild || !childProfiles || childProfiles.length < 2) return null;
  ...
  return (
    <>
      <button
        ref={btnRef}
        onClick={() => setOpen(o => !o)}
        className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full bg-white/15 hover:bg-white/25 border border-white/20 transition-colors"
        title="Switch child"
      >
        <span className="text-[11px] font-semibold text-white/90">Switch</span>
      ...
      {open && (
        <>
          ...
          {pos && (
            <div
              className="fixed bg-card border border-border rounded-2xl shadow-xl z-[9999] overflow-hidden w-64"
